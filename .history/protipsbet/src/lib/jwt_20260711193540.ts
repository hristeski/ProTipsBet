...
// .NET's ClaimTypes.Role serializes under this exact URI, not "role":
...
export function getRole(decoded: DecodedToken | null): string | undefined {
  if (!decoded) return undefined;
  return (decoded[ROLE_CLAIM_URI] as string | undefined) ?? decoded.role;
}
...
// Any authenticated, non-expired token - used to gate the customer account
// area (unlike isAdminToken, this doesn't care about role).
export function isLoggedIn(token: string | null): boolean {
  if (!token) return false;
  const decoded = decodeToken(token);
  return !!decoded && !isExpired(decoded);
}
