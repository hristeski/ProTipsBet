...
function CheckoutContent() {
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      form.append("transactionReference", txHash);
      form.append("planId", plan.id);
      form.append("paymentMethod", selectedMethod!);
      form.append("proof", proofFile!);
    ...
    <div className="pb-24 px-4 pt-8 max-w-lg mx-auto">
          ...
          {method.contactOnly ? (
            <div className="bg-neutral-950 p-4 rounded-xl mb-6">
              <p className="text-sm text-neutral-300 mb-3">
                Contact us first so we can share the secure payment details privately.
              </p>
              <div className="flex flex-col gap-2 text-sm">
                <a href={CONTACT_INFO.whatsapp} target="_blank" rel="noopener noreferrer" className="text-emerald-400 font-semibold hover:underline">
                  WhatsApp: {CONTACT_INFO.whatsapp}
                </a>
                <a href={`mailto:${CONTACT_INFO.email}`} className="text-emerald-400 font-semibold hover:underline">
      ...
      {step === "manual-account" && method && (
        <form onSubmit={handleFinalSubmit} className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
          ...
          <div className="mb-6">
            <label className="block text-xs font-bold text-neutral-400 mb-2 uppercase tracking-wider">Transaction Hash (TXID)</label>
            <input
              type="text"
              required
              value={txHash}
              onChange={(e) => setTxHash(e.target.value)}
              placeholder="0x... или blockchain transaction ID"
              className="w-full px-4 py-3 bg-neutral-950 border border-neutral-800 rounded-xl focus:outline-none focus:border-emerald-500 text-white transition-colors"
            />
          </div>
