...
import { CheckCircle2, XCircle, Clock, ImageOff, RefreshCw, ShieldAlert, MessageCircle } from "lucide-react";
...
// wa.me needs digits only - strips spaces, dashes, parentheses, leading +
          ...
          {payments.map((p) => (
            ...
            >
              <div className="flex items-start justify-between gap-4 mb-4">
                <div>
                  <p className="font-bold text-white">{p.userFullName || p.userEmail}</p>
                  <p className="text-xs text-neutral-500">{p.userEmail}</p>
                  {p.userWhatsApp && (
                    <a
                      href={toWhatsAppLink(p.userWhatsApp)}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 text-xs font-bold text-[#25D366] hover:underline mt-1"
                    >
                      <MessageCircle size={12} /> {p.userWhatsApp}
                    </a>
                  )}
