...
import { CheckCircle, TrendingUp, ImageIcon, Trophy, Calendar, Loader2, X } from "lucide-react";
      ...
      <div className="flex justify-center md:justify-start gap-3 mb-10">
        {(["all", "vip", "free"] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-6 py-2.5 rounded-lg text-sm font-black transition-all uppercase tracking-wide ${
              filter === f 
              ? "bg-emerald-500 text-neutral-950 shadow-[0_0_15px_rgba(16,185,129,0.3)]" 
              : "bg-neutral-900 text-neutral-400 hover:bg-neutral-800 hover:text-white"
            }`}
          >
            {f === "all" ? "All Tickets" : `${f} Tickets`}
          </button>
        ))}
      </div>
          ...
          <AnimatePresence>
            {filtered.map((ticket, index) => (
              ...
              >
                ...
                {/* Ticket Header */}
                <div className="p-5 border-b border-neutral-800 bg-neutral-900/50 flex justify-between items-start relative z-10">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      {ticket.isVip ? (
                        <span className="bg-amber-500 text-black px-2 py-0.5 rounded text-xs font-black uppercase flex items-center gap-1"><Trophy size={12}/> VIP</span>
                      ) : (
                        <span className="bg-emerald-500 text-black px-2 py-0.5 rounded text-xs font-black uppercase">FREE</span>
                      )}
                    ...
                    <h3 className="text-xl font-black text-white">{ticket.description}</h3>
                  </div>
                  <div className="text-right bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-4 py-2">
                    <p className="text-emerald-500 text-xs font-black uppercase tracking-wider mb-0.5">Total Odds</p>
                    <p className="text-2xl font-black text-white">@{ticket.totalOdds}</p>
                  </div>
                </div>

                {/* Direct Match Table (No Accordion) */}
                ...
                <div className="p-4 border-t border-neutral-800 bg-neutral-950/50 relative z-10">
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 text-sm font-bold text-neutral-400 hover:text-white py-2 hover:bg-neutral-900 rounded-lg transition-all"
                  >
                    <ImageIcon size={16} className="text-emerald-500"/>
                    VIEW ORIGINAL TICKET IMAGE
                  </button>
                </div>
              </motion.div>
            ))}
