...
export default function AdminTipsPage() {
  ...
  const updateResult = async (id: number, result: string) => {
    ...
    fetchTips(); // Освежи табела по промена
  };
  ...
  const handleCreateTip = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/admin/tips`, {
        ...
        body: JSON.stringify({
          ...newTip,
          // Парсирање на датумот за C#
          matchDate: new Date(newTip.matchDate).toISOString() 
        })
      });

      if (res.ok) {
        setIsModalOpen(false); // Затвори модал
        fetchTips(); // Освежи табела
        // Ресетирај форма
        setNewTip({
          homeTeam: "", awayTeam: "", matchDate: "", predictionType: "", odds: 1.50, isVip: false, isPublished: true, analysis: ""
        });
      } else {
        alert("Грешка при зачувување на натпреварот.");
      }
    } catch (err) {
      console.error(err);
    } finally {
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
            <div className="flex justify-between items-center p-4 border-b border-neutral-800 bg-neutral-950">
              <h2 className="font-black text-white text-lg">Add New Tip</h2>
              <button onClick={() => setIsModalOpen(false)} className="text-neutral-500 hover:text-white"><X size={20} /></button>
            </div>
            
            <form onSubmit={handleCreateTip} className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-neutral-400 mb-1 uppercase">Home Team</label>
                  <input type="text" required value={newTip.homeTeam} onChange={e => setNewTip({...newTip, homeTeam: e.target.value})} className="w-full px-3 py-2 bg-neutral-950 border border-neutral-800 rounded-lg text-white focus:border-emerald-500 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-neutral-400 mb-1 uppercase">Away Team</label>
                  <input type="text" required value={newTip.awayTeam} onChange={e => setNewTip({...newTip, awayTeam: e.target.value})} className="w-full px-3 py-2 bg-neutral-950 border border-neutral-800 rounded-lg text-white focus:border-emerald-500 focus:outline-none" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" required value={newTip.predictionType} onChange={e => setNewTip({...newTip, predictionType: e.target.value})} className="w-full px-3 py-2 bg-neutral-950 border border-neutral-800 rounded-lg text-white focus:border-emerald-500 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-neutral-400 mb-1 uppercase">Odds</label>
                  <input type="number" step="0.01" required value={newTip.odds} onChange={e => setNewTip({...newTip, odds: parseFloat(e.target.value)})} className="w-full px-3 py-2 bg-neutral-950 border border-neutral-800 rounded-lg text-white focus:border-emerald-500 focus:outline-none" />
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-neutral-400 mb-1 uppercase">Match Date & Time</label>
                <input type="datetime-local" required value={newTip.matchDate} onChange={e => setNewTip({...newTip, matchDate: e.target.value})} className="w-full px-3 py-2 bg-neutral-950 border border-neutral-800 rounded-lg text-white focus:border-emerald-500 focus:outline-none [color-scheme:dark]" />
              </div>

              <div className="flex gap-6 py-2">
                <label className="flex items-center gap-2 text-sm text-white font-bold cursor-pointer">
                  <input type="checkbox" checked={newTip.isVip} onChange={e => setNewTip({...newTip, isVip: e.target.checked})} className="w-4 h-4 accent-emerald-500" />
                  VIP Match
                </label>
                <label className="flex items-center gap-2 text-sm text-white font-bold cursor-pointer">
                  <input type="checkbox" checked={newTip.isPublished} onChange={e => setNewTip({...newTip, isPublished: e.target.checked})} className="w-4 h-4 accent-emerald-500" />
                  Publish Immediately
                </label>
              </div>
              ...
              <button type="submit" disabled={submitting} className="w-full flex justify-center items-center gap-2 py-3 bg-emerald-500 text-neutral-950 font-black rounded-lg hover:bg-emerald-400 transition-colors disabled:opacity-50 mt-4">
                {submitting ? <Loader2 size={18} className="animate-spin" /> : "Save Match"}
              </button>
            </form>
          </div>
        </div>
