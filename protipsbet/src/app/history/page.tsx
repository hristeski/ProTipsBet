...
import { CheckCircle2, XCircle, TrendingUp, Filter, Calendar, Camera, Trophy, Target, Zap } from "lucide-react";
...
export default function HistoryPage() {
  ...
  // Проширена база на тикети со различни варијанти
  const historyTickets = [
    {
      ...
      totalOdds: 12.50,
      status: "win",
      proofImage: "https://images.unsplash.com/photo-1540747913346-19e32dc3e97e?q=80&w=800&auto=format&fit=crop", 
      icon: Trophy,
      matches: [
    ...
    },
    {
      ...
      totalOdds: 2.85,
      status: "win",
      proofImage: "https://images.unsplash.com/photo-1518605368461-1e12522231ce?q=80&w=800&auto=format&fit=crop",
      icon: Target,
      matches: [
        { name: "Netherlands vs Italy", pick: "Netherlands (Asian Handicap -1)", status: "win" }
      ]
    },
    {
      id: "TKT-888",
      ...
      icon: Zap,
      matches: [
        { name: "Belgium vs Denmark", pick: "Over 2.5 Goals", status: "win" }
      ]
    },
    {
      id: "TKT-887",
    ...
    },
    {
      ...
      totalOdds: 2.40,
      status: "win",
      proofImage: "https://images.unsplash.com/photo-1574629810360-7efbb6b1531eb?q=80&w=800&auto=format&fit=crop",
      icon: Target,
      matches: [
        { name: "Japan vs South Korea", pick: "Draw (X)", status: "win" }
      ]
    },
    {
      id: "TKT-885",
  ...
  const filteredTickets = historyTickets.filter(
  ...
  return (
    <div className="pb-24 px-6 pt-24 md:pt-32 max-w-7xl mx-auto min-h-screen relative">
      ...
      {/* 2. FILTERS */}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="flex items-center gap-3 mb-10 overflow-x-auto pb-4 scrollbar-hide border-b border-zinc-800/50">
      ...
      {/* 3. TICKET LIST (CSS Grid for Desktop Optimization) */}
        ...
        <AnimatePresence mode="popLayout">
          {filteredTickets.map((ticket, index) => (
            ...
            >
              ...
              {/* Ticket Matches (Flex-grow ensures footer aligns at the bottom) */}
              ...
              {/* TICKET FOOTER: Proof Image & Odds */}
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center pt-5 border-t border-zinc-800 gap-4 mt-auto">
                <div className="flex-1">
                  {ticket.proofImage ? (
                    <button 
                      onClick={() => setExpandedImage(ticket.proofImage!)}
        ...
        {expandedImage && (
          ...
          >
            <motion.img 
              ...
              className="max-w-full max-h-[90vh] rounded-3xl border border-zinc-800 shadow-[0_0_50px_rgba(0,0,0,0.8)] object-contain"
            />
          </motion.div>
        )}
      </AnimatePresence>
